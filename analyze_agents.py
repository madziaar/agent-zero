        ...
        for root, dirs, files in os.walk(agent_path):
            ...
            for d in dirs:
                ...
                if rel_path not in structure[agent]:
                    structure[agent][rel_path] = {"type": "directory", "files": []}
            ...
            for f in files:
                ...
                if parent_dir not in structure[agent]:
                    structure[agent][parent_dir] = {"type": "directory", "files": []}
            ...
            for i, agent1 in enumerate(agents):
                for agent2 in agents[i+1:]:
                    try:
                        ...
                        print(f"Error comparing {rel_path} between {agent1} and {agent2}: {e}")
...
def suggest_optimizations(analysis):
        ...
        for agent in analysis["agent_profiles"]:
            ...
            for path in analysis["structure_analysis"]["detailed_structure"].get(agent, {}):
                ...
                for file in analysis["structure_analysis"]["detailed_structure"][agent][path].get("files", []):
                    agent_paths.add(os.path.join(path, file).replace("\\", "/"))
    ...
    common_files = analysis.get("structure_analysis", {}).get("common_paths", [])
    if common_files:
        template_structure = {}
...
def print_analysis_results(analysis, suggestions):
    ...
    for agent in analysis['agent_profiles']:
        prompt_count = analysis.get('prompt_analysis', {}).get('prompt_stats', {}).get(agent, {}).get('prompt_file_count', 0)
        unique_paths = analysis.get('structure_analysis', {}).get('stats', {}).get('unique_paths_per_agent', {}).get(agent, 0)
        print(f"  - {agent}: {prompt_count} prompt files, {unique_paths} unique files/directories")
    
    # Print common files
    common_paths = analysis.get('structure_analysis', {}).get('common_paths', [])
    ...
    divergent_files = analysis.get('prompt_analysis', {}).get('divergent_prompt_files', [])
    ...
    for file_data in divergent_files[:3]:  # Show just a few examples
        print(f"  - {file_data['file']} (used in {', '.join(file_data['agents'])})")
    if len(divergent_files) > 3:
        print(f"  - ... and {len(divergent_files) - 3} more")
    
    # Print similar files
    similar_files = analysis.get('similarity_analysis', {}).get('file_similarities', [])
    ...
    for file_data in sorted(similar_files, key=lambda x: x['average_similarity'], reverse=True)[:3]:
        print(f"  - {file_data['file']} (avg. similarity: {file_data['average_similarity']:.2f})")
...
def main():
    ...
    if choice.lower() == 'y':
        output_file = input("Enter output filename (default: agent_analysis.json): ")
        if not output_file:
